...
{
    if (picref->pts == AV_NOPTS_VALUE) return 0;
    AVFilterContext *ctx = inlink->dst;
    AVFilterLink *outlink = ctx->outputs[0];
    AssContext *ass = ctx->priv;
    int detect_change = 0;
    long long time_ms = llrint(picref->pts * av_q2d(inlink->time_base) * 1000);
    ASS_Image *image = ass_render_frame(ass->renderer, ass->track,
                                        time_ms, &detect_change);

    if (detect_change)
        av_log(ctx, AV_LOG_DEBUG, "Change happened at time ms:%lld\n", time_ms);
